...
bool AuctionBotSeller::Initialize()
{
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot seller filters:");
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Forced Inclusion %u items", (uint32)includeItems.size());
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Forced Exclusion %u items", (uint32)excludeItems.size());

	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading npc vendor items for filter..");
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Npc vendor filter has %u items", (uint32)npcItems.size());

	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading loot items for filter..");
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loot filter has %u items", (uint32)lootItems.size());
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Sorting and cleaning items for AHBot seller...");
    ...
    if (!itemsAdded)
    {
		sLog->outError(LOG_FILTER_SERVER_LOADING, "AuctionHouseBot seller not have items, disabled.");
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AuctionHouseBot seller will use %u items to fill auction house (according your config choices)", itemsAdded);

    LoadConfig();

	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Items loaded \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
		sLog->outInfo(LOG_FILTER_SERVER_LOADING, "\t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot seller configuration data loaded and initialized");
    return true;
}
...
void AuctionBotSeller::LoadSellerValues(SellerConfiguration& config)
{
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: minTime = %u", config.GetMinTime());
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: maxTime = %u", config.GetMaxTime());

	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: For AH type %u", config.GetHouseType());
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: GrayItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GRAY));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: WhiteItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_WHITE));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: GreenItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_GREEN));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: BlueItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_BLUE));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: PurpleItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_PURPLE));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: OrangeItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_ORANGE));
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: YellowItems = %u", config.GetItemsAmountPerQuality(AUCTION_QUALITY_YELLOW));
}
...
uint32 AuctionBotSeller::SetStat(SellerConfiguration& config)
{
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: Missed Item       \tGray\tWhite\tGreen\tBlue\tPurple\tOrange\tYellow");
    for (uint32 i = 0; i < MAX_ITEM_CLASS; ++i)
    {
		sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: \t\t%u\t%u\t%u\t%u\t%u\t%u\t%u",
...
void AuctionBotSeller::AddNewAuctions(SellerConfiguration& config)
{
    ...
    if (config.LastMissedItem > sAuctionBotConfig->GetItemPerCycleBoost())
    {
		...
		sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: Boost value used to fill AH! (if this happens often adjust both ItemsPerCycle in worldserver.conf)");
    }
    else
    ...
    while (GetItemsToSell(config, itemsToSell, allItems) && items > 0)
    {
        ...
        if (!itemId)
        {
			sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: Item entry 0 auction creating attempt.");
            continue;
        }
        ...
        if (!prototype)
        {
			sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: Unknown item %u auction creating attempt.", itemId);
            continue;
        }
        ...
        if (!item)
        {
			sLog->outError(LOG_FILTER_SERVER_LOADING, "AHBot: Item::CreateItem() returned NULL for item %u (stack: %u)", itemId, stackCount);
            return;
        }
	...
	sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: Added %u items to auction", count);
}
...
bool AuctionBotSeller::Update(AuctionHouseType houseType)
{
    ...
    {
		sLog->outInfo(LOG_FILTER_SERVER_LOADING, "AHBot: %s selling ...", AuctionBotConfig::GetHouseTypeName(houseType));
